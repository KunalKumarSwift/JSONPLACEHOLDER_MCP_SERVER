...
This project uses the Model Context Protocol (MCP) SDK for Node.js. For latest API documentation, refer to:
...
- `McpServer` class for high-level server management
- `registerTool()` method for registering tools with input schemas
- `StdioServerTransport` for stdio-based communication

The server is configured to run on stdio and can be debugged using VS Code with the `.vscode/mcp.json` configuration.
